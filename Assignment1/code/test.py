...
if __name__ == '__main__':
    ...
    try:
        ...
        swedTestData = dataset.get_swedish_test_corpus().parsed_sents()
        swedTp = TransitionParser.load('swedish.model')

        danishTestData = dataset.get_danish_test_corpus().parsed_sents()
        danishTp = TransitionParser.load('danish.model')

        swedParsed = swedTp.parse(swedTestData)
        danishParsed = danishTp.parse(danishTestData)

        with open('swed_test.conll', 'w') as f:
            for p in swedParsed:
                f.write(p.to_conll(10).encode('utf-8'))
                f.write('\n')

        with open('danish_test.conll', 'w') as f:
            for p in danishParsed:
                f.write(p.to_conll(10).encode('utf-8'))
                f.write('\n')

        ev = DependencyEvaluator(swedTestData, swedParsed)
        print "SWEED UAS: {} \n SWEED LAS: {}".format(*ev.eval())

        ev = DependencyEvaluator(danishTestData, danishParsed)
        print "DANISH UAS: {} \n DANISH LAS: {}".format(*ev.eval())
